...
test('the counter can not go below 0', () => {
  const counter = 0;
  const wrapper = setup(null, { counter });
...
test('the counter can not go below 0 and an error message is shown ', () => {
  const counter = 0;
  const wrapper = setup(null, { counter });
  ...
  expect(errMsg.text()).toBe('The counter cannot go below 0'); 
